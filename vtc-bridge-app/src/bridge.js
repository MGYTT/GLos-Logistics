...
// Czas oczekiwania zanim uznamy brak joba jako CANCELLED (nie delivered)
// ETS2 po delivery przez ~8-12s nie ma joba zanim pokaże summary screen
...
class Bridge {
  constructor(config, emit) {
    ...
    // Nowe: pending delivery state
    this._pendingDelivery     = null  // { jobKey, prevKey, timestamp, lastRaw }
  }
  ...
  async _tick() {
    ...
    // Jeśli mamy pending delivery (job właśnie zniknął)
    if (this._pendingDelivery) {
      ...
      if (hasJob && jobKey === this._pendingDelivery.jobKey) {
        // Ten sam job wrócił (np. błąd odczytu) → kontynuuj normalnie
        this._pendingDelivery = null
        this.lastJobKey = jobKey
      ...
      // Brak joba — czekamy na grace period
      if (elapsed < DELIVERY_GRACE_MS) {
        // W trakcie grace period — wysyłamy zwykły tick bez zdarzenia
        const payload = this._buildPayload(raw, 'none', null)
        await this._sendPayload(payload, raw, 'none', null)
        return
      }

      // Grace period minął bez nowego joba — sprawdź czy to delivery (ETS2 summary screen)
      // Jeśli gra jest załadowana ale nie ma joba — to DELIVERED (wróciliśmy do wolnej jazdy)
      // Jeśli truck.wipers lub inne oznaki normalnej jazdy — delivered
      // Uznajemy za DELIVERED jeśli gra jest connected i truck istnieje
      if (raw?.game?.connected && raw?.truck) {
        await this._fireDelivered(this._pendingDelivery, raw)
      } else {
        // Gra się zamknęła / crash → cancelled
    ...
    // ── Normalna logika (bez pending) ─────────────────────────────────
    let event = 'none'

    if (hasJob && jobKey !== this.lastJobKey && this.lastJobKey !== null) {
      // Zmiana joba bez przerwy → delivered + started
      event = 'job_delivered'
    } else if (hasJob && jobKey !== this.lastJobKey) {
      event = 'job_started'
    } else if (!hasJob && this.lastJobKey !== null) {
      // Job zniknął → NIE oznaczamy od razu jako cancelled!
      // Ustawiamy pending delivery i czekamy
      this._pendingDelivery = {
        jobKey:    this.lastJobKey,
        prevKey:   this.lastJobKey,
        timestamp: Date.now(),
        lastRaw:   raw,
      }
      this.lastJobKey = null
  ...
  // Wysyła zdarzenie job_delivered po grace period
  async _fireDelivered(pending, raw) {
    const { jobKey } = pending
    const [originCity, destinationCity] = jobKey.split('→')

    const payload = this._buildPayloadDelivered(raw, {
  ...
  // Wysyła zdarzenie job_cancelled
  ...
  _buildPayloadDelivered(raw, deliveredJob) {
    const { game, truck, trailer, navigation } = raw ?? {}
    ...
    return {
      ...
      delivered_job: {
        ...
        distance_km:      distKm,
        income:           null,   // income z telemetrii (prev)
        fuel_used:        fuelUsed,
  ...
  async _sendOffline() {
    try {
      await this._post(`${this.config.server_url}/api/bridge`, {
        ...
        active_job: null, event: 'none', telemetry: { has_job: false },
      })
    } catch {}
  }
  ...
  _get(url) {
    return new Promise((resolve, reject) => {
      const req = http.get(url, { timeout: 4000 }, res => {
        let d = ''
        res.on('data', c => (d += c))
        res.on('end',  () => { try { resolve({ body: JSON.parse(d) }) } catch { reject(new Error('Parse error')) } })
      })
      req.on('error',   reject)
