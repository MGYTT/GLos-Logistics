...
import {
  ...
  TrendingDown, Users, Plus, Minus, ChevronDown,
...
import {
  adjustBalance, transferBalance,
  resetWallet, setFuelPrice,
...
interface Transaction {
  ...
  created_at:    string
  member: { username: string; avatar_url: string | null; rank: string } | null
}
...
export function AdminWalletClient({
  wallets, recentTransactions, stats,
}: Props) {
  ...
  const filteredWallets = useMemo(() =>
    data.wallets.filter(w =>
      !search || w.member?.username.toLowerCase().includes(search.toLowerCase())
    ),
    [data.wallets, search]
  )
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
                ...
                <div key={w.member_id}
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-zinc-800
                                    ...
                                    text-sm font-bold text-amber-400 shrink-0
                                    overflow-hidden">
                      {w.member?.avatar_url ? (
                        <img src={w.member.avatar_url} alt=""
                          className="w-full h-full object-cover" />
                      ) : (
                        (w.member?.username[0] ?? '?').toUpperCase()
                      )}
                    </div>
      ...
      {tab === 'history' && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {['all', 'job_pay', 'manual_credit', 'manual_debit', 'fine', 'shop_purchase'].map(f => (
              <button
                key={f}
              ...
              {filteredTx.map(tx => (
                <div key={tx.id}
                  ...
                  <div className="w-8 h-8 rounded-full bg-zinc-800
                    ...
                    {tx.member?.avatar_url ? (
                      <img src={tx.member.avatar_url} alt=""
                        className="w-full h-full object-cover" />
                    ) : (
                      (tx.member?.username[0] ?? '?').toUpperCase()
                    )}
                  </div>
      ...
      {tab === 'tools' && (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          ].map(({ modal, icon: Icon, title, description, color, bg, border }) => (
            <button
              key={modal}
      ...
      <AnimatePresence>
        {modal && (
          <OperationModal
            ...
            onDone={() => {
              // Odśwież dane po operacji — full page reload
