...
export function MapDriverPanel({ driver, onClose }: Props) {
  const m   = driver.memberInfo
  const cfg = m ? getRankConfig(m.rank as any) : null
  const secAgo = Math.floor((Date.now() - driver.lastSeen) / 1000)
    ...
    >
      <div className="bg-zinc-900/97 backdrop-blur-xl border border-zinc-700/80
        ...
        >
          ...
          <div className="relative shrink-0">
            {m?.avatar_url ? (
              ...
              >
                {(m?.username ?? driver.name)[0].toUpperCase()}
              </div>
            )}
          ...
          {[
            {
              icon: Gauge,
              label: 'Prędkość',
              value: '— km/h',    // Trucky API nie zwraca prędkości
              color: 'text-blue-400',
              bg:    'bg-blue-500/10',
            },
            {
        ...
        <div className="px-4 py-2.5 border-t border-zinc-800/80
          ...
          <span className="text-xs text-zinc-600">
            Aktualizacja:{' '}
            {secAgo < 6 ? 'przed chwilą' : `${secAgo}s temu`}
          </span>
        </div>
      </div>
    </motion.div>
