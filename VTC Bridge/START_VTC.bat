...
if not exist "%~dp0vtc-bridge.js"      ( echo  [BLAD] Brak pliku: vtc-bridge.js      & set MISSING_FILES=1 )
if not exist "%~dp0vtc-verify.js"      ( echo  [BLAD] Brak pliku: vtc-verify.js      & set MISSING_FILES=1 )
if not exist "%~dp0vtc-save-config.js" ( echo  [BLAD] Brak pliku: vtc-save-config.js & set MISSING_FILES=1 )
...
echo   Znajdziesz go w: Panel VTC - Profil - API Bridge
echo.
echo  ══════════════════════════════════════════════
echo.
...
:ask_server
set VTC_SERVER=
set /p VTC_SERVER="  Adres serwera VTC (https://glos-logistics.vercel.app): "
if "%VTC_SERVER%"=="" ( echo  [BLAD] Adres nie moze byc pusty! & goto :ask_server )
...
if "%VTC_API_KEY%"=="" ( echo. & echo  [BLAD] Klucz nie moze byc pusty! & echo. & goto :ask_apikey )
...
if "%VERIFY_RESULT%"=="" (
    ...
    if /i "%TRY_AGAIN%"=="T" ( echo. & goto :ask_server )
    exit /b 1
)
...
echo.
set /p TRY_AGAIN="  Sprobowac ponownie? (T/N): "
if /i "%TRY_AGAIN%"=="T" ( echo. & goto :ask_apikey )
exit /b 1
...
if %errorlevel% neq 0 (
    echo  [WARN] Funbit nie odpowiada — Bridge bedzie czekac na gre
) else (
...
echo.
echo  ══════════════════════════════════════════════
echo   VTC Bridge uruchomiony! Nie zamykaj tego okna.
...
:: KLUCZOWE: uruchom node w NOWYM oknie konsoli z pełnym TTY
:: Dzięki temu stdin jest prawdziwym TTY a nie pipe z BAT
:: /WAIT = czekaj aż okno zostanie zamknięte
start "VTC Bridge — Nie zamykaj!" /WAIT cmd /c "node vtc-bridge.js & pause"

:: ── Po zamknięciu okna Bridge ──────────────────────────────
echo.
echo  ══════════════════════════════════════════════
